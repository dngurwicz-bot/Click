...
function ItemModal({ listKey, item, onClose, onSaved }: ItemModalProps) {
  ...
  async function handleSave() {
    ...
    try {
      if (isNew) {
        await api.post(`/api/org/lookups/${listKey}/items`, {
        ...
        await api.put(`/api/org/lookups/${listKey}/items/${item!.id}`, {
          label_he:   form.label_he.trim(),
          sort_order: Number(form.sort_order),
...
export default function OrgLookupDetailPage() {
  ...
  const loadData = useCallback(() => {
    setLoading(true);
    api.get<LookupListOut>(`/api/org/lookups/${listKey}`)
      .then(setData)
      .catch(console.error)
